...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-black text-green-400 shadow-lg overflow-hidden min-h-[297mm] font-mono px-10 py-10 flex flex-col">
        ...
        @if ($links->isNotEmpty())
            <section class="mt-6">
                <p class="text-xs text-green-600">user@cv:~$ cat links.txt</p>
                <div class="mt-2 border border-green-500/60 px-6 py-4 space-y-1 text-xs">
                    @foreach ($links as $link)
                        <p>
                            <span class="text-green-300">{{ $link->name }}:</span>
