...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-white text-stone-800 shadow-lg min-h-[297mm] font-sans px-14 py-12 flex flex-col">
        ...
        {{-- Hoved --}}
        <header class="flex items-start justify-between gap-10 pb-8 border-b border-stone-200">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-[0.45em] text-[#4a2545]">Curriculum Vitae</p>
        ...
        <div class="mt-9 flex-1 space-y-10">
            ...
            @if (isset($coverLetter))
                ...
                <section>
                    <div class="flex items-center gap-4 border-b border-stone-200 pb-3">
                        <span class="w-8 h-8 rounded-full border-2 border-[#4a2545] text-[#4a2545] text-xs font-bold grid place-items-center shrink-0">{{ str_pad($section, 2, '0', STR_PAD_LEFT) }}</span>
                        <h2 class="text-sm font-bold uppercase tracking-[0.25em] text-stone-900">Ansøgning</h2>
                    </div>
            ...
            @elseif ($jobs)
                ...
                <section>
                    <div class="flex items-center gap-4 border-b border-stone-200 pb-3">
                        <span class="w-8 h-8 rounded-full border-2 border-[#4a2545] text-[#4a2545] text-xs font-bold grid place-items-center shrink-0">{{ str_pad($section, 2, '0', STR_PAD_LEFT) }}</span>
                        <h2 class="text-sm font-bold uppercase tracking-[0.25em] text-stone-900">Erhvervserfaring &amp; Uddannelse</h2>
                    </div>
                    <div class="mt-6 space-y-6">
                        @foreach ($jobs as $job)
                            <article class="pl-1">
                                ...
                                <p class="text-sm text-[#4a2545]">{{ $job['company'] }}</p>
            ...
            @if ($skills)
                ...
                <section>
                    <div class="flex items-center gap-4 border-b border-stone-200 pb-3">
                        <span class="w-8 h-8 rounded-full border-2 border-[#4a2545] text-[#4a2545] text-xs font-bold grid place-items-center shrink-0">{{ str_pad($section, 2, '0', STR_PAD_LEFT) }}</span>
                        <h2 class="text-sm font-bold uppercase tracking-[0.25em] text-stone-900">Kompetencer</h2>
                    </div>
                    <ul class="mt-6 grid grid-cols-3 gap-x-8 gap-y-2.5 text-sm text-stone-700">
                        @foreach ($skills as $skill)
                            <li class="flex items-start gap-2.5">
                                <span class="mt-[7px] w-1.5 h-1.5 rounded-full bg-[#4a2545] shrink-0"></span>
                                <span>{{ $skill }}</span>
                            </li>
                        @endforeach
                    </ul>
            ...
            @if ($links->isNotEmpty())
                ...
                <section>
                    <div class="flex items-center gap-4 border-b border-stone-200 pb-3">
                        <span class="w-8 h-8 rounded-full border-2 border-[#4a2545] text-[#4a2545] text-xs font-bold grid place-items-center shrink-0">{{ str_pad($section, 2, '0', STR_PAD_LEFT) }}</span>
                        <h2 class="text-sm font-bold uppercase tracking-[0.25em] text-stone-900">Links</h2>
                    </div>
                    <ul class="mt-6 flex flex-wrap gap-x-12 gap-y-3 text-sm">
                        @foreach ($links as $link)
                            <li>
                                <p class="font-bold text-stone-900">{{ $link->name }}</p>
                                <a href="{{ $link->url }}" class="text-[#4a2545] underline underline-offset-2 break-all">{{ $link->prettifyUrl() }}</a>
                            </li>
                        @endforeach
                    </ul>
                </section>
