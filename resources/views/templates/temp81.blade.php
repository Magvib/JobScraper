...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-[#efe9e2] shadow-lg min-h-[297mm] font-sans text-[#43302b]">
        ...
        <div class="px-12 py-8">
            ...
            @if ($links->isNotEmpty())
                <section class="mt-8">
                    <h2 class="text-sm font-extrabold uppercase tracking-widest text-[#b45b4d] px-1">Links</h2>
                    <ul class="mt-4 space-y-1.5 text-sm text-[#5c433d]">
                        @foreach ($links as $link)
                            <li>
                                <span class="font-semibold">{{ $link->name }}:</span>
                                <a href="{{ $link->url }}" class="text-[#b45b4d] underline break-all">{{ $link->prettifyUrl() }}</a>
                            </li>
                        @endforeach
                    </ul>
                </section>
