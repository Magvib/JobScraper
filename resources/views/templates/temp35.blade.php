    ...
    <div class="cv-page max-w-[210mm] mx-auto bg-white shadow-lg overflow-hidden grid grid-cols-[1fr_60mm] min-h-[297mm] font-sans text-gray-900">
        ...
        <main class="px-12 py-12">
            ...
            @if (isset($coverLetter))
                <section class="mt-10">
                    <h2 class="text-sm font-bold uppercase tracking-[0.2em] text-gray-900 border-b-2 border-orange-500 pb-2">Ansøgning</h2>
        ...
        <aside class="bg-gray-900 text-gray-100 px-7 py-12 flex flex-col gap-8">
            ...
            @if ($links->isNotEmpty())
                <section>
                    <h2 class="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400 border-b border-white/10 pb-2">Links</h2>
                    <ul class="mt-4 space-y-3 text-sm">
                        @foreach ($links as $link)
                            <li>
                                <p class="text-gray-400 text-xs uppercase tracking-wide">{{ $link->name }}</p>
                                <a href="{{ $link->url }}" class="mt-0.5 block text-orange-300 underline break-all">{{ $link->prettifyUrl() }}</a>
                            </li>
                        @endforeach
                    </ul>
                </section>
