...
<head>
    ...
    <style>
        ...
        /* L-formede hjørnemærker som på et teknisk tegningsark */
        .corner { position: absolute; width: 14mm; height: 14mm; border-color: #0f5257; }
        .corner-tl { top: 6mm; left: 6mm; border-top: 2px solid; border-left: 2px solid; }
        .corner-tr { top: 6mm; right: 6mm; border-top: 2px solid; border-right: 2px solid; }
        .corner-bl { bottom: 6mm; left: 6mm; border-bottom: 2px solid; border-left: 2px solid; }
        .corner-br { bottom: 6mm; right: 6mm; border-bottom: 2px solid; border-right: 2px solid; }
    </style>
</head>
@php
    $jobs = $user->cv_json;
...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-white text-slate-800 shadow-lg min-h-[297mm] font-sans relative px-16 py-14 flex flex-col">
        ...
        <header class="flex items-start justify-between gap-10">
            ...
            @if ($photo)
                <img src="{{ $photo }}" alt="{{ $user->name }}"
                     class="w-28 h-28 object-cover border border-slate-300 shrink-0 mt-1">
            @endif
        </header>

        {{-- Kontakt-række --}}
        <div class="mt-7 border-y border-slate-200 py-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-600">
            @if ($user->phone)
        ...
        <div class="mt-9 grid grid-cols-[1fr_50mm] gap-12 flex-1">
            ...
            <main>
                @if (isset($coverLetter))
                    <h2 class="text-[10px] font-bold uppercase tracking-[0.35em] text-[#0f5257] border-b border-slate-200 pb-2.5">Ansøgning</h2>
                    <div class="mt-6 space-y-4">
                        {!! $coverLetter->renderContext() !!}
                    </div>
                @elseif ($jobs)
                    <h2 class="text-[10px] font-bold uppercase tracking-[0.35em] text-[#0f5257] border-b border-slate-200 pb-2.5">Erhvervserfaring &amp; Uddannelse</h2>
                    <div class="mt-6 space-y-6">
                        @foreach ($jobs as $job)
                            <article class="grid grid-cols-[8mm_1fr] gap-4">
                                <span class="mt-2 w-2.5 h-2.5 bg-[#0f5257]"></span>
                                <div>
            ...
            @if ($skills || $links->isNotEmpty())
                <aside>
                    @if ($skills)
                        <h2 class="text-[10px] font-bold uppercase tracking-[0.35em] text-[#0f5257] border-b border-slate-200 pb-2.5">Kompetencer</h2>
                    ...
                    @if ($links->isNotEmpty())
                        <h2 class="mt-8 text-[10px] font-bold uppercase tracking-[0.35em] text-[#0f5257] border-b border-slate-200 pb-2.5">Links</h2>
