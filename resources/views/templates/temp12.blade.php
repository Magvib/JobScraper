...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-[#faf3e3] shadow-lg min-h-[297mm] font-mono text-stone-800 px-12 py-12">
        ...
        @if (isset($coverLetter))
            <section class="mt-10">
                <h2 class="text-sm font-bold uppercase tracking-[0.3em] text-stone-700 border-b border-dashed border-stone-500 pb-2">§ 1 — Ansøgning</h2>
        ...
        @if ($links->isNotEmpty())
            <section class="mt-10">
                <h2 class="text-sm font-bold uppercase tracking-[0.3em] text-stone-700 border-b border-dashed border-stone-500 pb-2">§ 3 — Links</h2>
                <ul class="mt-5 space-y-2 text-sm">
                    @foreach ($links as $link)
                        <li>
                            <span class="font-bold text-stone-700">{{ $link->name }}:</span>
                            <a href="{{ $link->url }}" class="text-stone-600 underline break-all">{{ $link->prettifyUrl() }}</a>
                        </li>
                    @endforeach
                </ul>
            </section>
