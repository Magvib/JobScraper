...
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-[#071f19] shadow-2xl min-h-[297mm] font-sans text-emerald-50 px-12 py-12">
        ...
        @if ($links->isNotEmpty())
            <section class="mt-9">
                <h2 class="text-xs font-bold uppercase tracking-[0.3em] text-emerald-300 border-b border-emerald-800/60 pb-2">Links</h2>
                <ul class="mt-4 space-y-1.5 text-sm text-emerald-50/80">
                    @foreach ($links as $link)
                        <li>
                            <span class="font-semibold text-white">{{ $link->name }}:</span>
                            <a href="{{ $link->url }}" class="text-emerald-300 underline break-all">{{ $link->prettifyUrl() }}</a>
                        </li>
                    @endforeach
                </ul>
            </section>
