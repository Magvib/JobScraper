...
@php
    ...
    $links = $user->links;
    $photo = $user->getImage();
    $initials = collect(explode(' ', trim((string) $user->name)))->filter()->map(fn ($p) => mb_strtoupper(mb_substr($p, 0, 1)))->take(2)->implode('');
@endphp
<body class="bg-white min-h-screen">
    <div class="cv-page max-w-[210mm] mx-auto bg-white text-stone-800 shadow-lg min-h-[297mm] font-sans flex flex-col">

        {{-- Monogram-felt med stort initial --}}
        <header class="flex items-stretch">
            <div class="w-[52mm] bg-[#374151] text-white grid place-items-center shrink-0">
                @if ($photo)
                    <img src="{{ $photo }}" alt="{{ $user->name }}" class="w-28 h-28 rounded-full object-cover ring-2 ring-white/70">
                @else
                    <span class="font-serif text-6xl font-bold tracking-widest">{{ $initials }}</span>
                @endif
            </div>
            <div class="flex-1 border-b-2 border-stone-300 px-10 py-8 min-w-0">
                <p class="text-[10px] font-bold uppercase tracking-[0.45em] text-[#374151]">Curriculum Vitae</p>
                <h1 class="mt-2 font-serif text-5xl font-bold leading-tight text-stone-900">{{ $user->name }}</h1>
                @if ($user->job_title)
        ...
        {{-- Kontakt-række --}}
        <div class="border-b border-stone-200 bg-stone-50 px-12 py-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-stone-600">
            @if ($user->phone)
        ...
        <div class="px-12 py-10 grid grid-cols-[1fr_50mm] gap-12 flex-1">
                    ...
                    <div class="mt-6 space-y-6">
                        @foreach ($jobs as $job)
                            <article>
                                ...
                                <p class="text-sm text-[#374151] font-medium">{{ $job['company'] }}</p>
            ...
            @if ($skills || $links->isNotEmpty())
                <aside>
                        ...
                        <ul class="mt-5 space-y-2.5 text-sm text-stone-700">
                            @foreach ($skills as $skill)
                                <li class="flex items-start gap-2.5">
                                    <span class="mt-[7px] w-1.5 h-1.5 bg-[#374151] shrink-0"></span>
                                    <span>{{ $skill }}</span>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    @if ($links->isNotEmpty())
                        <h2 class="mt-8 text-[10px] font-bold uppercase tracking-[0.35em] text-stone-900 border-b-2 border-stone-900 pb-2.5">Links</h2>
                        <ul class="mt-5 space-y-3 text-sm">
                            @foreach ($links as $link)
                                <li>
                                    <p class="font-bold text-stone-900">{{ $link->name }}</p>
                                    <a href="{{ $link->url }}" class="text-[#374151] underline underline-offset-2 break-all">{{ $link->prettifyUrl() }}</a>
                                </li>
                            @endforeach
                        </ul>
                    @endif
